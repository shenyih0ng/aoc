...
void _get_neighbours (Pos og_pos, vector<Pos>& neighbours, Pos curr_pos, size_t dim) {
...
size_t get_num_active (map<Pos, bool>& mem, 
		       vector<Pos>& neighbours) {
	size_t num_active = 0;
...
bool get_next_pos_state (Pos& pos, 
		         vector<Pos>& neighbours, 
		         map<Pos, bool>& mem) {	
	...
	for (;mIt != mem.end(); mIt++) {
		...
		for (; vIt != neighbours.end(); vIt++) {
			...
			if (mem.find(n_pos) == mem.end()) {
				vector<Pos> n_neighbours = get_neighbours(n_pos);
				if (get_next_pos_state(n_pos, n_neighbours, mem)) {
					next_state.insert(make_pair(n_pos, true));
				}
			}
		}
	}
...
int main (int argc, char* argv[]) {
	...
	if (input_stream.is_open()) {
		...
		// Part 1
		cout << "[p1]: " << run_n_cycles(start_state, NUM_CYCLES) << endl;
		...
		cout << "[p2]: " << run_n_cycles(n_start_state, NUM_CYCLES) << endl;
	}
