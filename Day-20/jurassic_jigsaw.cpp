...
class Tile {
       ...
       public:
        ...
        char get_pixel(int h_offset, int v_offset) {
                return image[v_offset][h_offset];
        };
        ...
        friend ostream& operator<<(ostream& out, Tile* t) {
                ...
                out << "no. sides matched: " << t->get_num_sides_matched()
                    << endl;
...
bool Tile::match(Tile* o_tile) {
        ...
        for (int num_check = 0; num_check < 4; num_check++) {
                ...
                flipped = true;
                matched_border_idx =
                    match_borders(t1_borders, t2_borders_flipped);
                if (matched_border_idx != -1) {
        ...
        if (matched_border_idx != -1) {
                ...
                o_tile->set_borders(
                    t2_borders);  // update border after transformation
...
class Dragon {
       public:
        ...
        Dragon(vector<pair<int, int>> coords, int w, int h)
            : coordinates{coords}, width{w}, height{h} {}
        ...
        void rotate() {
                ...
                for (; coordIt != coordinates.end(); coordIt++) {
                        ...
                        coordIt->first =
                            height - 1 - curr_coord.second;  // flipped y1 = x2
                }
        ...
        void flip() {
                ...
                for (; coordIt != coordinates.end(); coordIt++) {
                        coordIt->first =
                            width - 1 - coordIt->first;  // flipped x1 = x2
                }
        }
...
void find_dragon(Dragon* dragon, Tile* ul_tile,
                ...
                while (!row_end) {
                        ...
                        for (int idx = 0; idx < dragon_coords.size(); idx++) {
                                ...
                                Tile* target_tile = ul_tile->find_tile(
                                    (dragon_x_offset + coord.first) /
                                        IMAGE_SIZE,
                                    (dragon_y_offset + coord.second) /
                                        IMAGE_SIZE);
                                ...
                                dragon_occupied.insert(
                                    make_pair(dragon_x_offset + coord.first,
                                              dragon_y_offset + coord.second));
                        }

                        occupied_by_dragon.insert(dragon_occupied.begin(),
                                                  dragon_occupied.end());

                        dragon_x_offset++;
                        row_end = ul_tile->find_tile(
                                      (dragon_x_offset + dragon->width - 1) /
                                          IMAGE_SIZE,
                                      0) == nullptr;
                }

                dragon_y_offset++;
                end = ul_tile->find_tile(
                          0, (dragon_y_offset + dragon->height - 1) /
                                 IMAGE_SIZE) == nullptr;
        }
}
...
int main(int argc, char* argv[]) {
        ...
        vector<Tile*> tiles;
        int total_non_empty_pixels =
            0;  // keep track of number of non-empty pixels in image
        ...
        while (getline(input_stream, line)) {
                ...
                int tile_id =
                    stoi(line.substr(div_pos + 1, line.size() - div_pos + 2));
                ...
                while (num_lines < TILE_SIZE) {
                        ...
                        switch (num_lines) {
                                ...
                                case TILE_SIZE - 1:
                                        ...
                                        break;
                                default:
                                        string image_row = tile_row.substr(
                                            1, tile_row.size() - 2);
                                        image.push_back(image_row);
                                        total_non_empty_pixels +=
                                            count(begin(image_row),
                                                  end(image_row), '#');
                        }
        ...
        cout << "[p2]: " << total_non_empty_pixels - occupied_by_dragon.size()
             << endl;
