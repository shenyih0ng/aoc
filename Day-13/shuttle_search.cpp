...
mp::cpp_int get_earliest_timestamp (string bus_ids) {
	...
	while (getline(bus_ids_stream, bus_id, ',')) {
		if (bus_id != BUS_NOT_IN_SERVICE) {
			...
			N *= _bus_id;
			bId_delta.push_back(make_pair(_bus_id, _bus_id-delta));
		}
		delta++;
	}
	...
	for (int i=0; i < bId_delta.size(); i++) {
		...
		mp::cpp_int inv_NdNi = mp::powm(_NdNi, _Mi-2, _Mi); // all bus_ids are conveniently prime
...
int main (int argc, char* argv[]) {
	...
	if (input_stream.is_open()){
		...
		// Part 1	
		cout << "[p1]: " << get_bus_id_swait_time(d_ts, bus_ids) << endl;
