...
bool sort_cups(Cup* c1, Cup* c2) { return c1->value < c2->value; }
...
Cup* find_destination(Cup* curr_cup, vector<Cup*>& cups) {
        ...
        while (!found) {
                if (target < min_unselected->value) {
                        ...
                        Cup* possible_match =
                            cups[target - 1];  // cups is sorted anyways
...
void form_LL(string cups_str, Cup*& start_cup, Cup*& end_cup, Cup*& cup_L1,
             vector<Cup*>& cups) {
        Cup* prev_n = NULL;
