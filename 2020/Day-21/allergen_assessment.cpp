...
void step(set<string> foods, set<string> allergens,
          map<string, set<string>>& allergen_map, set<string>& found) {
        int initial_num_found = found.size();
        set<string>::iterator aIt = allergens.begin();
        for (; aIt != allergens.end(); aIt++) {
                map<string, set<string>>::iterator allergen_set =
                    allergen_map.find(*aIt);
                if (allergen_set == allergen_map.end()) {
                        ...
                        set_intersection(
                            ...
                            allergen_set->second.begin(),
                            allergen_set->second.end(),
                            inserter(intersect, intersect.begin()));

                        if (intersect.size() == 1) {
                                found.insert(intersect.begin(),
                                             intersect.end());
                                allergen_map[*aIt] = intersect;
                        } else {
                                // remove food that already has its allergen
                                // found from current set
                                set<string> diff;
                                set_difference(intersect.begin(),
                                               intersect.end(), found.begin(),
                                               found.end(),
        ...
        if (initial_num_found < found.size()) {
                ...
                for (; amIt != allergen_map.end(); amIt++) {
                        if (amIt->second.size() > 1) {
                                set<string> _diff;
                                set_difference(amIt->second.begin(),
                                               amIt->second.end(),
                                               found.begin(), found.end(),
                                               inserter(_diff, _diff.begin()));
                                if (_diff.size() == 1) {
                                        found.insert(_diff.begin(),
                                                     _diff.end());
                                }
                                amIt->second = _diff;
                        }
...
int main(int argc, char* argv[]) {
        ...
        if (input_stream.is_open()) {
                ...
                while (getline(input_stream, line)) {
                        ...
                        string allergens =
                            line.substr(apos + 10, epos - apos - 10);
