...
long int find_num_internal_arrangement(vector<int> subarray,
        ...
        while (curr_idx < subarray.size() - 1) {
                ...
                if (curr_val + next_val <= 3) {
                        ...
                        sub_subarray.insert(sub_subarray.begin(),
                                            subarray.begin(),
                                            subarray.begin() + curr_idx);
                        sub_subarray.push_back(curr_val + next_val);
                        sub_subarray.insert(sub_subarray.begin() + curr_idx + 1,
                                            subarray.begin() + curr_idx + 2,
                                            subarray.end());

                        // convert sub-subarray into string for lookup
                        ostringstream sub_subarr_str;
                        copy(sub_subarray.begin(), sub_subarray.end(),
                             ostream_iterator<int>(sub_subarr_str, ""));
                        string key = sub_subarr_str.str();
                        if (lookup.find(key) == lookup.end()) {
                                cout << "\t[sub-subarray]";
                                display(sub_subarray);
                                lookup.insert(key);
                                find_num_internal_arrangement(sub_subarray,
                                                              lookup);
                        }
                }
                ++curr_idx;
...
int main(int argc, char* argv[]) {
        ...
        if (input_stream.is_open()) {
                ...
                // Part 1
                cout << "part 1: " << calc_jolt_diff(ordered_set) << endl
                     << endl;

                // Part 2
                // The reason why part 1 asked for the jolt_diff_1 and
                // jolt_diff_3 is a hint to solving part 2 1) Construct a array
                // of jolt_diff 2) Using jolt_diff val of 3 as a "delimiter" ->
                // get all subarrays of jolt_diff of < 3 3) Calc all the
                // possible permutations within the subarray 4) Multiply num
                // permutations per subarray
                ...
                for (it = subarrays.begin(); it != subarrays.end(); ++it) {
                        ...
                        long int total_sub_arrangement =
                            1 + find_num_internal_arrangement(subarray, lookup);
                        cout << "sub arrangements: " << total_sub_arrangement
                             << endl
                             << endl;
