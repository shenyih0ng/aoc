...
int get_seat_helper(string& boarding_pass, int curr_idx, int lower, int upper) {
        ...
        if (c == 'F' || c == 'L') {
                ...
                return get_seat_helper(boarding_pass, curr_idx + 1, mid + 1,
                                       upper);
        }
...
int get_row(string& boarding_pass) {
        return get_seat_helper(boarding_pass, 0, 0, 127);
}
int get_col(string& boarding_pass) {
        return get_seat_helper(boarding_pass, 0, 0, 7);
}

int find_my_seat_id(set<int>& seat_ids) {
        for (set<int>::iterator it = seat_ids.begin(); it != seat_ids.end();
             it++) {
                int curr_seat_id = *it;
                it++;
                int next_seat_id = *it;
...
int main(int argc, char* argv[]) {
        ...
        // we could maintain a sorted vector/array with inser time log(n) but
        // insert time will stil be o(n)
        set<int> seat_ids;
