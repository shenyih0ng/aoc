...
void display_index(unordered_map<string, vector<string>>& index) {
        ...
        for (it = index.begin(); it != index.end(); ++it) {
                ...
                for (vector<string>::iterator v_it = parents.begin();
                     v_it != parents.end(); ++v_it) {
                        cout << "\t" << *v_it << endl;
                }
                cout << endl;
        }
...
void display_index(unordered_map<string, unordered_map<string, int>>& index) {
        ...
        for (it = index.begin(); it != index.end(); ++it) {
                ...
                for (children_itr = children_map.begin();
                     children_itr != children_map.end(); ++children_itr) {
                        cout << "\t" << children_itr->first << " "
                             << children_itr->second << endl;
                }
                cout << endl;
        }
}
...
void build_index(string& line,
                 unordered_map<string, unordered_map<string, int>>& index) {
        // Part 2: index of parent -> child
        // Need to consider 'leaf' bags 'contain no other bags' -> no
        // child_match means no other bags
        smatch root_match;
        if (regex_search(line, root_match, ROOT_RGX)) {
                string root = root_match[1];
                string children = root_match[2];
                ...
                while (regex_search(children, child_match, CHILD_RGX)) {
                        ...
                        unordered_map<string, unordered_map<string, int>>::
                            const_iterator got = index.find(root);
                        if (got != index.end()) {
                                unordered_map<string, int> children_map =
                                    got->second;  // TODO why does this not act
                                                  // like a reference
...
set<string> find_bags_contain(unordered_map<string, vector<string>>& index,
                              string target) {
        // Part 1
        unordered_map<string, vector<string>>::const_iterator target_bag =
            index.find(target);
        set<string> bags;
...
int find_num_bags_in_bag(
    unordered_map<string, unordered_map<string, int>>& index, string target) {
        unordered_map<string, unordered_map<string, int>>::const_iterator
            target_bag = index.find(target);
        int count = 1;  // to count the current target, however we will need to
                        // -1 to not consider the root target. hmmm
        if (target_bag != index.end()) {
                unordered_map<string, int> children_map = target_bag->second;
                if (children_map.size() > 0) {
                        unordered_map<string, int>::iterator it;
                        for (it = children_map.begin();
                             it != children_map.end(); ++it) {
                                string child = it->first;
                                int child_value = it->second;
                                count += (child_value *
                                          find_num_bags_in_bag(index, child));
                        }
                }
        } else {
...
int main(int argc, char* argv[]) {
        ...
        cout << "Num bags: " << find_bags_contain(index_p1, TARGET_BAG).size()
             << endl;
        cout << "Num bags: " << find_num_bags_in_bag(index_p2, TARGET_BAG) - 1
             << endl;
