...
void display(int idx, int line_length) {
        ...
        pos_pointer.insert(pos_pointer.begin(),
                           line_length - pos_pointer.length(), '-');
...
int move(istream& terrain, int& start_h_idx, move_op* moves, int* move_values) {
        ...
        while (terrain.peek() != EOF) {
                ...
                trees +=
                    last_action(terrain, line, last_move_value, start_h_idx);
        }
...
int main() {
        ...
        // int test_values[] = {3,1};
        // int total_trees = move(terrain, start_h_idx, move_operations,
        // test_values);
        ...
        for (int* test_value : test_values) {
                int start_h_idx = 0;
                int tree_encountered =
                    move(terrain, start_h_idx, move_operations, test_value);
