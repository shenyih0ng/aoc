...
pair<float, float>& operator+=(pair<float, float>& lhs,
                               pair<float, float>& rhs) {
        lhs.first += rhs.first;
...
void step(map<pair<float, float>, bool>& tiles,
        ...
        for (; bIt != black_tiles.end(); bIt++) {
                ...
                for (int idx = 0; idx < 6; idx++) {
                        pair<float, float> adj_pos =
                            (*bIt) + decode_step(adjacent[idx]);
...
int main(int argc, char* argv[]) {
        ...
        if (input_stream.is_open()) {
                ...
                // Part 2
                vector<pair<float, float>> black_tiles;
                map<pair<float, float>, bool>::const_iterator mIt =
                    tiles.begin();
