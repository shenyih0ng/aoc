...
void init_xB (unordered_map<mp::cpp_int, int>& xB, int* m) {
	...
	for (int iM = 0; iM < *m; iM++) {
		mp::cpp_int val = mp::powm(mp::cpp_int(A), mp::cpp_int(iM), mp::cpp_int(P));
		xB[val] = iM;
	}
}

int get_loop_size (int pubkey, int* m, mp::cpp_int* inv_Am, unordered_map<mp::cpp_int, int>& xB) {
	for (int _iM = 0; _iM < *m; _iM++) {
		// giant step
		mp::cpp_int _rhs = mp::powm(*inv_Am, mp::cpp_int(_iM), mp::cpp_int(P));
...
int main (int argc, char* argv[]) {
	...
	if (input_stream.is_open()) {
		...
		int _m = ceil(sqrt(P-1)); // since P is prime - phi(prime) = prime-1
		...
		mp::cpp_int inv_Am = mp::powm(mp::cpp_int(A), inv_Am_exp, mp::cpp_int(P));
		...
		mp::cpp_int ekey = powm(mp::cpp_int(card_pubkey), mp::cpp_int(door_ls), mp::cpp_int(P));
		cout << "[p1]: " << ekey << endl;
	}	
