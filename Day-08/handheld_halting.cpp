...
void display_cache(vector<pair<string, int>>& cache) {
        ...
        for (it = cache.begin(); it != cache.end(); ++it) {
                ...
                cout << "Op: " << op_pair.first
                     << " Accessed: " << op_pair.second << endl;
        }
}

void add_n_lines_to_cache(int n_lines, ifstream& stream,
...
void modify_last_accessed(int op_num, int modifier,
                          vector<pair<string, int>>& cache) {
        pair<string, int>& op_pair = cache[op_num];
        op_pair.second += modifier;
}

void change_op_tag(int op_num, string new_op_tag,
                   vector<pair<string, int>>& cache) {
        pair<string, int>& op_pair = cache[op_num];
        op_pair.first.replace(0, 3, new_op_tag);
}

void process_op(string op_str, ifstream& input_stream,
                vector<pair<string, int>>& cache, int& op_num, int& acc) {
...
bool run(ifstream& input_stream, vector<pair<string, int>>& cache, int op_num,
         int& acc) {
        bool error = false;
        while (op_num < static_cast<int>(cache.size()) && !error) {
                ...
                if (op.second == true) {
                        ...
                        cout << "\tOp: " << op.first << " Op_Num: " << op_num
                             << endl;
                        error = true;
                }
        }
...
bool is_corrupt(ifstream& input_stream, vector<pair<string, int>>& cache,
                int op_num, int& acc) {
        // TODO I dont think this is the best implementation (seems awfully
        // repetitive)
        bool error = false;
        ...
        while (op_num < static_cast<int>(cache.size()) && !error) {
                ...
                if (op.second == true) {
                        ...
                        for (it = accessed_op_nums.begin();
                             it != accessed_op_nums.end(); ++it) {
        ...
        while (op_num < static_cast<int>(cache.size()) && !found_corrupt_jmp) {
                ...
                if (op.second = true) {
                        ...
                        if (op_tag == "jmp") {
                                ...
                                if (is_corrupt(input_stream, cache, op_num,
                                               acc)) {
                                        modify_last_accessed(op_num, 1, cache);
                        ...
                        if (op_num >= cache.size()) {
                                lines_to_add = op_num - cache.size() + 1;
                                add_n_lines_to_cache(lines_to_add, input_stream,
                                                     cache);
                        }
                }
        }
...
int main(int argc, char* argv[]) {
        ...
        if (input_stream.is_open()) {
                ...
                cout << "Ended Run with ";
                if (!success) {
                        cout << "error(infinite loop detected) ";
                } else {
                        cout << "no error ";
                }
                ...
                if (!success) {
                        ...
                        int corrected_acc =
                            fix_corrupt_jmp(input_stream, cache);
                        cout << "corrected Acc Value: " << corrected_acc
                             << endl;
                }
        } else {
