...
bool if_repeated_hand (vector<int>& p1, vector<int>& p2, 
		vector<vector<int>>& prev_p1, vector<vector<int>>& prev_p2) {
	assert(prev_p1.size() == prev_p2.size());
	
	vector<vector<int>>::iterator p1_found = find(prev_p1.begin(), prev_p1.end(), p1);	
	vector<vector<int>>::iterator p2_found = find(prev_p2.begin(), prev_p2.end(), p2);	
	if (p1_found != prev_p1.end() && p2_found != prev_p2.end()) {
		return distance(p1_found, prev_p1.begin()) == distance(p2_found, prev_p2.begin());
	}
...
bool play (vector<int>& p1, vector<int>& p2,
		vector<vector<int>>& prev_p1, vector<vector<int>>& prev_p2) {
	while (!has_winner(p1,p2) && !if_repeated_hand(p1, p2, prev_p1, prev_p2)) {
		...
		if (if_subgame(p1, p2)) {
			vector<int> sub_p1;
			copy(p1.begin() + 1, p1.begin() + p1[0] + 1, back_inserter(sub_p1));
			vector<int> sub_p2;
			copy(p2.begin() + 1, p2.begin() + p2[0] + 1, back_inserter(sub_p2));

			vector<vector<int>> sub_prev_p1, sub_prev_p2;
			p1_round_win = play(sub_p1, sub_p2, sub_prev_p1, sub_prev_p2);
		} else {
...
int main (int argc, char* argv[]) {
	...
	if (input_stream.is_open()){
		...
		cout << " wscore: " << calc_score((part1_p1_win)? p1 : p2) << endl;
		...
		cout << " wscore: " << calc_score((part2_p1_win)? p1_copy : p2_copy) << endl;
	}
