...
/*
 * A wacky implementation of infix expression evaluation (*purely for learning/fun sake)
 *
 * - It does not account for operator precedence (treats every operator as same precedence)
 * - Does not convert infix expression into postfix/prefix as a intermediate form for calcuation
 *
...
cpp_int eval (string eq) {
	...
	while (idx < eq.size()) {
		...
		if (idx == eq.size()-1 || !is_digit(c)) {
			...
			if (c == ')') {
				Operand* operand_pval = (Operand*)ops.top(); // resulting val in parentheses
				...
				ops.push((c == '(' || c == ')') ? new Op() : new Operator(c));
			}
		}
		idx++;
	}
