...
template <class T>
void read_starting_numbers (ifstream& input, T& spoken, size_t& idx, size_t& last ) {
	string val_str;
	size_t val;
 ...
 * using unordered_map
 */
size_t find_val (unordered_map<size_t, size_t>& spoken, size_t& last, size_t& cIdx, size_t end_pos) {
...
int main (int argc, char* argv[]) {
	...
	/* allocate memory in heap since size of array will be over stack limit (ulimit)*/
	size_t* spoken = new size_t[END_1 < END_2 ? END_2 : END_1]();  // 1-2s
	read_starting_numbers(input_stream, spoken, idx, last);
	
	// p1
	printf("end_pos: %d val: %d\n", END_1, find_val(spoken, last, idx, END_1));
	// p2
	printf("end_pos: %d val: %d\n", END_2, find_val(spoken, last, idx, END_2));
}
