...
vector<pair<int, int>> extract_rule(string rule_line) {
        ...
        regex_iterator<string::iterator> rit(rule_line.begin(), rule_line.end(),
                                             rules_rx);
...
vector<pair<int, int>> reduce(vector<pair<int, int>> r1,
                              vector<pair<int, int>> r2) {
        vector<pair<int, int>> reduced;
        reduced.insert(reduced.begin(), r1.begin(),
                       r1.end());  // treat r1 as base
...
bool rnode::is_leaf() { return cl == 0 && cr == 0; }
...
int find_error_rate(vector<int> ticket, vector<pair<int, int>>& rules) {
        ...
        for (int idx = 0; idx < ticket.size(); idx++) {
                ...
                for (int rIdx = 0; rIdx < rules.size(); rIdx++) {
                        ...
                        bool _valid = (ticket_val <= rule.second) &&
                                      (ticket_val >= rule.first);
                        if (_valid) {
                                valid = true;
                                break;
                        }
...
int find_error_rate(vector<vector<int>> tickets,
                    vector<pair<int, int>>& rules) {
        int t_error_rate = 0;
...
vector<vector<int>> get_valid_tickets(vector<vector<int>>& tickets,
                                      vector<pair<int, int>>& rules) {
...
unordered_map<int, int> get_ticket_mapping(vector<vector<int>> tickets,
                                           rnode* rTree) {
        ...
        for (int col = 0; col < nCols; col++) {
                ...
                valid_rIdx.insert(valid_rIdx.begin(),
                                  col);  // pos 0 is reserved col value
                col_valid_rules.push_back(valid_rIdx);
        }

        sort(col_valid_rules.begin(), col_valid_rules.end(),
             _compare_col_rules);
...
int main(int argc, char* argv[]) {
        ...
        if (input_stream.is_open()) {
                ...
                vector<vector<int>> valid_tickets =
                    get_valid_tickets(nearby_tickets, root->rules);
                valid_tickets.push_back(my_ticket);

                long int departX_val = 1;
                unordered_map<int, int> ticket_mapping =
                    get_ticket_mapping(valid_tickets, root);
                for (int idx = 0; idx < rule_names.size(); idx++) {
                        string rule_name = rule_names[idx];
                        int sep_pos = rule_name.find_first_of(' ');
                        if (sep_pos != -1) {
                                if (rule_name.substr(0, sep_pos) ==
                                    "departure") {
                                        departX_val *=
                                            my_ticket[ticket_mapping[idx]];
                                }
                        }
                }
