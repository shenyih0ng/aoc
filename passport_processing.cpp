...
int is_valid_field (string kv_line) {	
	...
	if (REQUIRED_FIELDS.find(key) != REQUIRED_FIELDS.end()) {
		field_validator validate_func = REQUIRED_FIELDS.find(key) -> second;
		bool valid = validate_func(value);
		if (valid == 0) {
			cout << "Key: " << key << " Value: " << value << " Result: " << valid << endl;
		}
...
int main () {
	...
	if (input_stream.is_open()) {
		...
		while ( getline(input_stream, line) ) {
			...
			passport_kv.insert(passport_kv.end(), kv.begin(), kv.end());
