...
vector<unsigned long int> get_xtra_mem_addr (string mask, int bMem) {
	...
	for (int idx = 0; idx < permutations.size(); idx+=float_bit_pos.size()) {
...
int main (int argc, char* argv[])
{
	...
	if (input_stream.is_open()) {
		...
		while(getline(input_stream, line)) {
			if (!strncmp(line.c_str(), MASK_PREFIX.c_str(), MASK_PREFIX.size())) {
				// mask
				mask = line.substr(3+4);
			} else {
				// mem
				unsigned long int cbrac_pos = line.find_first_of("]");
				unsigned long int mem_pos = stoi(line.substr(4, cbrac_pos-1));
 				unsigned long int mem_value_int = stoi(line.substr(cbrac_pos + 4));

				// v1
				//bitset<36> mem_value_bit(mem_value_int);
				//bitset<36> bitmask = prep_mask(mask, mem_value_bit);
				//unsigned long int masked_value = ((mem_value_bit^bitmask)^mem_value_bit).to_ulong();
				//sum = sum - memory[mem_pos] + masked_value;
				//memory[mem_pos] = masked_value;
				
				// v2
				vector<unsigned long int> mem_addrs = get_xtra_mem_addr(mask, mem_pos);
				for (int idx=0; idx < mem_addrs.size(); idx++) {
					sum = sum - memory[mem_addrs[idx]] + mem_value_int;
